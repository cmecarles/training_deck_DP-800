...
- Population is asynchronous: poll `FULLTEXTCATALOGPROPERTY(cat, 'PopulateStatus')` = 0 or `sys.dm_fts_index_population` before trusting fresh rows; `NO POPULATION` means empty until `ALTER FULLTEXT INDEX ... START FULL POPULATION`.
