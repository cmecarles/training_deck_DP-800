...
- **Requirement 6** — nothing is installed on a VM. (SQL Insights (preview), which relied on a Telegraf monitoring VM, was **retired on 31 December 2024**; Azure SQL Analytics is a legacy workspace solution "no longer in active development". The current agentless estate-level option is **database watcher**, which stores data in Azure Data Explorer/Fabric rather than a Log Analytics workspace — a valid extra, but not required here.)
...
- Retired/legacy: SQL Insights (preview, monitoring VM) retired 31 Dec 2024; Azure SQL Analytics is legacy. Current agentless estate monitoring: **database watcher** (managed identity, ADX/Fabric store).
